...
pub fn is_hw_frame(fmt: ff::format::Pixel) -> bool {
    matches!(fmt, ff::format::Pixel::VIDEOTOOLBOX | ff::format::Pixel::VAAPI)
}
...
mod tests {
    ...
    fn vp8_and_legacy_codecs_stay_on_software() {
        assert!(!codec_is_hw_eligible(ff::codec::Id::VP8), "VP8 carve-out (§10)");
        ...
        'outer: for (s, packet) in ictx.packets() {
            ...
            while decoder.receive_frame(&mut decoded).is_ok() {
                ...
                if let Some(sw) = transfer_if_hw(&decoded).expect("transfer") {
                    eprintln!("  transferred -> {:?} {}x{}", sw.format(), sw.width(), sw.height());
                }
                if frames >= 3 {
                    break 'outer;
                }
