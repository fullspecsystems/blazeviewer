...
// ---------------------------------------------------------------------------
// The FFmpeg→MF audio locator bridge (task #99, Windows).
// ---------------------------------------------------------------------------

/// Stamp each of `ff`'s audio tracks with its Media Foundation twin's locator, so a
/// picker row on the Windows runtime catalog (FFmpeg's — it supersedes MF's because MF
/// models no subtitle tracks) can reach the WASAPI engine, whose currency is
/// [`TrackLocator::MfStream`] reader stream indices.
///
/// This exists because **MF enumerates streams in a different order than the
/// container/FFmpeg** — measured twice, from the enumeration side (`mf_tracks`) and the
/// decode side (`mf_audio`): on the multitrack fixture MF hands back the `fra` track as
/// its first audio stream where FFmpeg reads `eng` first. So an index can never cross
/// the seam raw; tracks are matched on what they *are* instead:
///
/// 1. **(language, codec family, channels)** — languages compared by primary subtag
///    resolved through [`language_display`]'s table, since the backends report the same
///    language in different forms (`"eng"` vs `"en"`); codecs compared as the
///    [`audio_codec_display`] family, since MF can't see FFmpeg's profile ints and
///    collapses e.g. every DTS flavour to `"dts"`.
/// 2. Leftovers match on **(codec family, channels)** alone, but only where the
///    languages don't positively disagree — a missing language on one side (MF often
///    has none) is not a disagreement.
///
/// Within a tier, candidates pair **in order** — the agreed fallback for genuinely
/// identical tracks (same language, codec, channels). That residual ambiguity is real
/// and documented rather than papered over: two indistinguishable tracks can still
/// cross if the backends order them differently, and nothing observable tells them
/// apart. An FFmpeg track with no MF twin is simply left on its `FfStream` locator —
/// the shell then refuses the switch honestly instead of guessing.
pub fn bridge_mf_audio_locators(ff: &mut MediaTrackCatalog, mf: &MediaTrackCatalog) {
    struct Cand {
        lang: Option<String>,
        family: String,
        channels: u16,
    ...
    let mut cands: Vec<Cand> = mf
        .audio
        .tracks
        .iter()
        .filter_map(|t| {
            let locator = mf.locator(t.id)?.clone();
            if !matches!(locator, TrackLocator::MfStream(_)) {
                return None;
            }
            Some(Cand {
                lang: t.language.as_deref().map(bridge_lang_key),
                family: audio_codec_display(&t.codec_raw, None),
                channels: t.audio.as_ref().map_or(0, |a| a.channels),
                locator,
                taken: false,
            })
        })
    ...
    let ff_keys: Vec<(u64, Option<String>, String, u16)> = ff
        .audio
        .tracks
        .iter()
        .map(|t| {
            (
                t.id.local_id,
                t.language.as_deref().map(bridge_lang_key),
                audio_codec_display(&t.codec_raw, None),
                t.audio.as_ref().map_or(0, |a| a.channels),
            )
        })
        .collect();

    let mut assign: Vec<(u64, TrackLocator)> = Vec::new();
    let mut leftovers: Vec<usize> = Vec::new();
    for (i, (local_id, lang, family, channels)) in ff_keys.iter().enumerate() {
        match cands
            .iter_mut()
            .find(|c| !c.taken && c.lang == *lang && c.family == *family && c.channels == *channels)
        {
            Some(c) => {
                c.taken = true;
                assign.push((*local_id, c.locator.clone()));
            }
            None => leftovers.push(i),
        }
    }
    for i in leftovers {
        let (local_id, lang, family, channels) = &ff_keys[i];
        if let Some(c) = cands.iter_mut().find(|c| {
            !c.taken
                && c.family == *family
                && c.channels == *channels
                // A missing language is unknown, not different; only a positive
                // disagreement blocks the pairing.
                && (c.lang.is_none() || lang.is_none() || c.lang == *lang)
        }) {
            c.taken = true;
            assign.push((*local_id, c.locator.clone()));
        }
    }
    for (local_id, locator) in assign {
        ff.set_locator(local_id, locator);
    }
}

/// A language tag reduced to a backend-neutral match key: the primary subtag, resolved
/// through the display table where known (`"eng"`, `"en"`, `"en-US"` → `"English"`), or
/// passed through lowercased so unknown-but-equal tags still match each other.
fn bridge_lang_key(tag: &str) -> String {
    let primary = tag
        .trim()
        .split(['-', '_'])
        .next()
...
mod tests {
    ...
    // -- the FFmpeg→MF audio locator bridge (task #99) -----------------------

    fn audio(gen: u64, local_id: u64, lang: Option<&str>, codec_raw: &str, ch: u16) -> MediaTrack {
        MediaTrack {
            id: TrackId {
            ...
            title: None,
            codec_raw: codec_raw.to_string(),
            codec: audio_codec_display(codec_raw, None),
            capability: TrackCapability::Playable,
            flags: TrackFlags::none(),
    ...
    /// `ff` side: audio tracks whose `local_id` is the stream index, FfStream locators.
    fn ff_catalog(tracks: Vec<MediaTrack>) -> MediaTrackCatalog {
        let ids: Vec<u64> = tracks.iter().map(|t| t.id.local_id).collect();
        let mut c = MediaTrackCatalog::new(
            7,
            MediaBackend::FFmpeg,
        ...
        for id in ids {
            c.set_locator(id, TrackLocator::FfStream(id as usize));
        }
        c
    }

    /// `mf` side: audio tracks + the MF reader stream index each sits on.
    fn mf_catalog(tracks: Vec<(MediaTrack, u32)>) -> MediaTrackCatalog {
        let locs: Vec<(u64, u32)> = tracks.iter().map(|(t, s)| (t.id.local_id, *s)).collect();
        let mut c = MediaTrackCatalog::new(
            7,
            MediaBackend::MediaFoundation,
            TrackSet::complete(tracks.into_iter().map(|(t, _)| t).collect()),
            TrackSet::unavailable(),
        );
        for (id, s) in locs {
            c.set_locator(id, TrackLocator::MfStream(s));
        }
        c
    }

    /// The measured multitrack case: FFmpeg reads `[eng, fra]`, MF enumerates the same
    /// two tracks the other way round. The bridge must pair by identity, not by order —
    /// order-crossing is the entire reason it exists.
    #[test]
    fn the_bridge_matches_by_identity_not_order() {
        let mut ff = ff_catalog(vec![
            audio(7, 1, Some("eng"), "aac", 2),
            audio(7, 2, Some("fra"), "aac", 2),
        ]);
        let mf = mf_catalog(vec![
            (audio(7, 0, Some("fra"), "aac", 2), 0),
            (audio(7, 1, Some("eng"), "aac", 2), 1),
        ]);
        bridge_mf_audio_locators(&mut ff, &mf);
        assert_eq!(
            ff.locator(ff.track_id(1)),
            Some(&TrackLocator::MfStream(1)),
            "the eng row must reach MF's eng stream"
        );
        assert_eq!(ff.locator(ff.track_id(2)), Some(&TrackLocator::MfStream(0)));
    }

    /// The backends report the same language in different forms (`"eng"` vs `"en"`,
    /// spike-measured on AVFoundation and equally true of MF's BCP-47 tags), and MF
    /// collapses codec detail FFmpeg keeps (every DTS flavour → `"dts"`; PCM flavours).
    /// Matching is on the resolved language + the display family, so none of that
    /// blocks a pair.
    #[test]
    fn the_bridge_resolves_language_and_codec_forms() {
        let mut ff = ff_catalog(vec![
            audio(7, 1, Some("eng"), "dts", 6),
            audio(7, 2, Some("fra"), "pcm_s24le", 2),
        ]);
        let mf = mf_catalog(vec![
            (audio(7, 0, Some("fr-FR"), "pcm_s16le", 2), 0),
            (audio(7, 1, Some("en"), "dts", 6), 1),
        ]);
        bridge_mf_audio_locators(&mut ff, &mf);
        assert_eq!(ff.locator(ff.track_id(1)), Some(&TrackLocator::MfStream(1)));
        assert_eq!(ff.locator(ff.track_id(2)), Some(&TrackLocator::MfStream(0)));
    }

    /// Two genuinely identical tracks (same language, codec, channels) fall back to
    /// within-group order — the documented residual ambiguity, applied deterministically.
    #[test]
    fn identical_tracks_pair_in_order() {
        let mut ff = ff_catalog(vec![
            audio(7, 1, Some("eng"), "aac", 2),
            audio(7, 2, Some("eng"), "aac", 2),
        ]);
        let mf = mf_catalog(vec![
            (audio(7, 0, Some("eng"), "aac", 2), 0),
            (audio(7, 1, Some("eng"), "aac", 2), 1),
        ]);
        bridge_mf_audio_locators(&mut ff, &mf);
        assert_eq!(ff.locator(ff.track_id(1)), Some(&TrackLocator::MfStream(0)));
        assert_eq!(ff.locator(ff.track_id(2)), Some(&TrackLocator::MfStream(1)));
    }

    /// MF often reports no language at all (`MF_SD_LANGUAGE` absent). A missing language
    /// is unknown — the codec/channel tier still pairs it, with channels disambiguating.
    #[test]
    fn a_missing_mf_language_still_pairs_by_codec_and_channels() {
        let mut ff = ff_catalog(vec![
            audio(7, 1, Some("eng"), "aac", 2),
            audio(7, 2, Some("fra"), "aac", 6),
        ]);
        let mf = mf_catalog(vec![
            (audio(7, 0, None, "aac", 6), 0),
            (audio(7, 1, None, "aac", 2), 1),
        ]);
        bridge_mf_audio_locators(&mut ff, &mf);
        assert_eq!(ff.locator(ff.track_id(1)), Some(&TrackLocator::MfStream(1)));
        assert_eq!(ff.locator(ff.track_id(2)), Some(&TrackLocator::MfStream(0)));
    ...
    /// A positive language disagreement blocks the fallback tier: better an unswitchable
    /// row than one that plays the wrong language while ticking the right one — the
    /// exact failure the bridge exists to prevent.
    #[test]
    fn conflicting_languages_never_pair_and_unmatched_rows_keep_ffstream() {
        let mut ff = ff_catalog(vec![audio(7, 1, Some("eng"), "aac", 2)]);
        let mf = mf_catalog(vec![(audio(7, 0, Some("fra"), "aac", 2), 0)]);
        bridge_mf_audio_locators(&mut ff, &mf);
        assert_eq!(
            ff.locator(ff.track_id(1)),
            Some(&TrackLocator::FfStream(1)),
            "an unbridged row keeps its FfStream locator (the shell refuses honestly)"
        );
    }
}
