//! Auto-update surface. **Windows-only work:** PhotoBlaze ships there as a per-user Velopack
//! install with built-in auto-update. macOS ships a notarized DMG (tag-driven GitHub release) and
//! Linux has no updater, so both compile the no-op [`stub`] below — which also keeps the `velopack`
