...
/// Linux image-only write. `rgba` is straight-alpha `width×height` RGBA8, exactly the layout
/// `arboard::ImageData` expects; arboard encodes it for the platform pasteboard.
#[cfg(all(unix, not(target_os = "macos")))]
pub fn set_image(width: u32, height: u32, rgba: &[u8]) -> Result<(), String> {
