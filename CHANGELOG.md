...
### Added
- **Undo a delete with `Ctrl+Z` (`⌘Z` on macOS).** Sent a photo to the Trash / Recycle Bin by
  mistake? Press `Ctrl+Z` / `⌘Z` (or Edit ▸ Undo Delete) to restore it — the photo comes back in
  place and PhotoBlaze jumps to it with a "Restored <name>" message. Works even if it was the last
  photo in the folder, on all platforms. (A permanent delete — `Shift+Del`, or `⌥⌘⌫` on macOS —
  can't be undone.)
...
- **`Del` / `⌘⌫` no longer silently loses a photo when the Recycle Bin / Trash is unavailable.** On
  a Windows drive set to skip the Recycle Bin (Recycle Bin Properties ▸ "Don't move files to the
  Recycle Bin"), or a macOS volume with no Trash (read-only or network), pressing `Del` / `⌘⌫` now
  asks you to confirm a permanent delete — the same prompt as `Shift+Del` / `⌥⌘⌫` — instead of
  quietly deleting (Windows) or just failing (macOS). When a file really is gone for good, the
  on-screen icon shows a permanent delete rather than a recycle.
