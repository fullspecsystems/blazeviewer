...
- **Touch works.** On a touchscreen, tapping the picture now does what tapping
  ought to do: the tap lands where your finger did, folder-tree rows open, and
  sliding a finger pans the image exactly as dragging with the left mouse button
  does. Until now a finger did nothing at all on a Windows touch display — the
  window is handed native touch the moment the system has a touchscreen, which
  stops Windows from quietly turning taps into mouse clicks, and nothing was
  ...
  - **Press and hold** opens the photo's context menu, the way a right-click
    does with a mouse.
  - A tap is allowed a little wobble before it counts as a pan, so the picture
    stops creeping out from under your finger on a deliberate press.
  - The first finger down steers; a second finger, or a palm resting on the
    glass, can't take the photo away from it. Touching a panel and touching the
    photo stay separate, so opening the thumbnail strip no longer confuses one
    for the other.

- **See how an AI image was made.** Open the details panel (`Shift+I`) on a
  picture generated by ComfyUI or an Automatic1111-style tool and a
  **Generation** section now leads the table: the prompt and negative prompt,
  the model and any LoRAs, the seed, steps, CFG, sampler and size, plus a line
  for each refinement pass. Ordinary photos are untouched.
  - It reports only what the file actually says. Where a prompt was assembled by
    the workflow rather than typed in — so the exact wording was never saved —
