#!/usr/bin/env bash
# Publish the macOS Blaze Viewer DMG + Sparkle appcast to the downloads.blazeviewer.app feed —
...
# repoints BlazeViewer-latest.dmg → the uploaded DMG so /latest/mac serves the newest.
# Idempotent: re-running with the same DMG just re-uploads and re-points to the same target.
#
# Usage:
#   scripts/release-mac-upload.sh                       # newest BlazeViewer-*.dmg in dist/
...
REMOTE_DIR="/var/www/downloads.blazeviewer.app/mac"
while [[ $# -gt 0 ]]; do
	case "$1" in
...
# 1) Locate the DMG: explicit --dmg, else the newest BlazeViewer-*.dmg in --source (never the alias).
if [[ -n "$DMG" ]]; then
	[[ -f "$DMG" ]] || { echo "error: --dmg not found: $DMG" >&2; exit 1; }
else
	DMG="$(ls -t "$SOURCE"/BlazeViewer-*.dmg 2>/dev/null | grep -v 'BlazeViewer-latest.dmg' | head -1 || true)"
	[[ -n "$DMG" ]] || { echo "error: no BlazeViewer-*.dmg in $SOURCE (pass --dmg <path> or --source <dir>)" >&2; exit 1; }
fi
DMG_DIR="$(cd "$(dirname "$DMG")" && pwd)"
NAME="$(basename "$DMG")"
[[ "$NAME" != "BlazeViewer-latest.dmg" ]] || { echo "error: refusing to publish the 'latest' alias; need a versioned DMG" >&2; exit 1; }
...
echo "==> repoint BlazeViewer-latest.dmg -> $NAME"
ssh -o BatchMode=yes "$UPLOAD_HOST" "cd '$REMOTE_DIR' && ln -sfn '$NAME' BlazeViewer-latest.dmg && ls -l BlazeViewer-latest.dmg"
