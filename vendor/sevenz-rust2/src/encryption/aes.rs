...
// PhotoBlaze patch (candidate for upstream): cache the derived AES key across
// blocks.
//
// The 7z key derivation iterates SHA-256 over (salt ‖ password ‖ counter)
// 2^num_cycles_power times — ~10–40 ms at the standard cycles=19, scaling with
// password length. The salt lives in each block's coder properties, but 7-Zip
// writes ONE salt for the whole archive, so a non-solid encrypted archive (one
// block per file) re-derives the *identical* key for every single file: on a
// many-thousand-file stored+encrypted photo archive the derivations dominate
// the entire extraction (measured 3.9 s vs 0.15 s for 7-Zip, which caches, on
// 3000 files). The cache holds the single most recent derivation, keyed by a
// SHA-256 fingerprint of (cycles, salt, password) so no raw password bytes are
// retained. Concurrent first misses may derive redundantly in parallel (same
// result; last write wins) — that is bounded by the thread count, not the file
// count, and avoids serializing real work behind a lock.
...
fn derive_key_cached(num_cycles_power: u8, salt: &[u8], password: &[u8]) -> [u8; 32] {
    ...
    if let Some((cached_fp, key)) = KEY_CACHE.lock().unwrap().as_ref() {
