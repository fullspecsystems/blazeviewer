...
### HUD / toast icons — Font Awesome duotone (codified workflow)
...
change, never per frame (off the photo hot path). Icons are **Font Awesome Pro
`duotone`** SVGs (the house style): one tint (white) + the secondary layer's built-in
`.4` opacity carries the two-tone depth. They're vendored into the repo and rasterized
via the same `resvg`/`usvg`/`tiny-skia` stack `pb-decode` uses (`icon::rasterize` —
`currentColor`→white, then the text's black-outline pass for legibility).

**To add an icon:**
1. Find it in the local FA library:
   `D:\Media\fontawesome-pro-plus-7.3.0-web\svgs\duotone\<name>.svg`
   (`ls svgs/duotone | grep <kw>` to search; always the `duotone` weight).
...
**Licensing:** FA **Pro** assets (duotone is Pro) are licensed to the owner but **not
redistributable**. The repo is **private**, so vendoring the SVGs is in-bounds. If it
ever goes public: git-ignore `icons/` and load from the local FA path at build, or
swap the public build to free-tier solid/regular icons. (Privacy task #2 is unaffected
— the SVGs are compile-time assets, not a viewing trace.)
